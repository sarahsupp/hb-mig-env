...
Est3MigrationDates = function(dat){
  ...
  for (s in species) {
   ...
   spring_threshold = min(filter(dat_subset, DAY %in% c(1:151))$lat_se*2.56 + filter(dat_subset, DAY %in% c(1:151))$lat)
   fall_threshold = min(filter(dat_subset, DAY %in% c(213:364))$lat_se*2.56 + filter(dat_subset, DAY %in% c(213:364))$lat)
    ...
    #FIXME: An issue is also occurring due to all the NA values, I think. Find a slightly different way to index through values?
...
spring <- ggplot(dates, aes(YEAR, SPRING)) + 
 ...
 # geom_hline(yintercept = end_winter, linetype="dashed", col="hotpink") +
  theme_bw() + xlab("Year") + ylab("spring begin migration date") + 
...
peak <- ggplot(dates, aes(YEAR, MAXLAT)) + 
  ...
  geom_smooth(method="lm") +
 # geom_hline(yintercept = end_winter, linetype="dashed", col="hotpink") +
  theme_bw() + xlab("Year") + ylab("peak latitude (breeding) date") + 
  theme(text = element_text(size=14), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
  #annotate(geom="text", x= 2012, y= 55, label=paste0("median doy = ", round(end_winter,2)))
  facet_wrap(~species)
...
fall <- ggplot(dates, aes(YEAR, FALL)) + 
  ...
  theme_bw() + xlab("Year") + ylab("fall end migration date") + 
  theme(text = element_text(size=14), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
...
ggsave(filename = "figures/allspp_migrationDates.png", height = 11, width=8)
```
...
Peak latitude (popualation is no longer moving strongly northwards) appears to be fairly flat for most species across the time series. 
